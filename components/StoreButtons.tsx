...
export default function StoreButtons({
  ...
  const iosFirst = platform === "ios" || platform === "both";

  const sizeClasses =
    size === "large"
      ? "h-14 px-7 text-base gap-3"
      : "h-12 px-5 text-sm gap-2.5";
    ...
    <a
      ...
      className={`inline-flex items-center justify-center rounded-xl font-medium transition-all duration-200 hover:scale-[1.03] ${sizeClasses} ${
        (iosFirst ? true : false)
          ...
          : "border border-border bg-surface-alt text-foreground hover:border-accent/40"
      }`}
    >
      <AppleIcon />
  ...
  return (
    <div
      className={`flex flex-col sm:flex-row gap-3 w-full sm:w-auto ${className}`}
    >
      {iosFirst ? buttons : buttons.reverse()}
    </div>
  );
}
